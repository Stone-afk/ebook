...
func TestConnPool(t *testing.T) {
	ebook, err := gorm.Open(mysql.Open("root:root@tcp(localhost:13316)/ebook"))
	require.NoError(t, err)
	err = ebook.AutoMigrate(&Interactive{})
	require.NoError(t, err)
	interactive, err := gorm.Open(mysql.Open("root:root@tcp(localhost:13316)/ebook_interactive"))
	require.NoError(t, err)
	err = interactive.AutoMigrate(&Interactive{})
	require.NoError(t, err)
	db, err := gorm.Open(mysql.New(mysql.Config{
		Conn: &DoubleWritePool{
			src:     ebook.ConnPool,
			dst:     interactive.ConnPool,
