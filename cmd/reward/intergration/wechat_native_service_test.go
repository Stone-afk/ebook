...
import (
	...
	pmtmocks "ebook/cmd/api/proto/gen/payment/v1/mocks"
	"ebook/cmd/reward/domain"
	"ebook/cmd/reward/intergration/startup"
...
func (s *WechatNativeRewardServiceTestSuite) TestPreReward() {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			defer ctrl.Finish()
			// TODO accountClient 为 nil
			svc := startup.InitWechatNativeSvc(tc.mock(ctrl), nil)
